...
@section('content')
 ...
 <div class='row'>
        <div class="col-md-6">
            ...
            <!-- Form input -->
            {!! Form::model($member, ['url' => 'team/' . $member->id, 'method' => 'patch', 'files' => true, 'class' => 'form-horizontal']) !!}
                <div class="panel panel-flat">
                    <div class="panel-heading">
                        <h5 class="panel-title text-semibold">Edit Team Member</h5>
                    ...
                    <div class="panel-body">
                        <p class="content-group">The social media inputs can be filled by visiting that persons social account and copying the URL from the address bar. Accolades will be appear on the website as a nice list under the team member description. <strong>Changes are only applied when Update is clicked.</strong></p>
                        ...
                        {{-- Ugly loop that explodes the accolades string and puts each resulting element in a nicely syled input with individual remove buttons --}}
                        @foreach(explode('-,-', $member->accolades) as $accolade)
                            @if($loop->first)
                                <div class="form-group has-feedback has-feedback-left">
                                    <label class="col-lg-3 control-label col-sm-12">Accolades</label>
                                    <div class="col-lg-7 col-sm-9">
                                        <input type="text" name="accolades[]" class="form-control" placeholder="Enter accolade" value="{{ $accolade }}">
                                        <div class="form-control-feedback">
                                            <i class=" icon-trophy2"></i>
                                        </div>  
                                        <span class="help-block">(Optional) - Make sure you click Update to save changes</span>
                                    </div>
                            ...
                            @endif
                                        @if(! $loop->first)
                                            <div class="additional-accolade row"><br>
                                                <label class="col-lg-3 control-label col-sm-12">&nbsp;</label>
                                                <div class="col-lg-7 col-sm-9">
                                                    <input name="accolades[]" type="text" class="form-control" placeholder="Enter accolade" value="{{ $accolade }}">
                                                    <div class="form-control-feedback">
                                                        <i class=" icon-trophy2"></i> 
                                                    </div> 
                                                </div> 
                                                <div class="col-lg-2 col-sm-3" style="margin-top: 0;">
                                                    <button type="button" class="btn btn-danger btn-sm remove-accolade-input"><i class=" icon-cancel-circle2"></i></button> 
                                                </div>
                                            </div> 
                                        @endif   
                                        @if($loop->last)
